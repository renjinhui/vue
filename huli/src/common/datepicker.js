/*
    基于jq 的 datepicker 扩展插件
    @params  element    dom     选定节点
    @params  options    json    可选参数
    @options format     string  日期格式 默认'mm/dd/yyyy'
    @options weekStart  int     一周开始是哪一天 默认为0，周日开始排列
    @options language   string  显示的语言 支持 ZH 和 EN，默认'ZH'中文
    @options todayBtn   boolean 是否显示今天按钮 默认显示
*/
!function( $ ) {
    if(!$) return;
    ...
    var Datepicker = function(element, options){
        ...
        this.language = (options.language||'zh').toLowerCase();
        this.todayBtn = options.todayBtn == undefined ? true : options.todayBtn;
    ...
    Datepicker.prototype = {
        ...
        fillDow: function(){
            ...
            var html = '<tr>';
            var isZH = this.language == 'zh';
            while (dowCnt < this.weekStart + 7) {
                var day = isZH ? DPGlobal.dates.daysMin[(dowCnt++)%7] : DPGlobal.dates.daysShort[(dowCnt++)%7];
        ...
        fillMonths: function(){
            ...
            var isZH = this.language == 'zh';
            while (i < 12) {
                var month = isZH ? DPGlobal.dates.months[i++] : DPGlobal.dates.monthsShort[i++];
        ...
        fill: function() {
            var d = new Date(this.viewDate),
                ...
                isZH = this.language == 'zh';
            this.picker.find('.datepicker-days th:eq(1)')
                .text((isZH ? DPGlobal.dates.months[month] : DPGlobal.dates.monthsShort[month])+' '+year);
            ...
            this.todayBtn && html.push('<tr><td colspan="7" class="today day">'+ (isZH ? '今天' : 'Today') +'</td></tr>');
