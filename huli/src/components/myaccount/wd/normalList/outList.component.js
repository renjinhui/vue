...
const wdOutList = React.createClass({
  ...
  transFn(item){
    ...
    if (item.transferStatus == 1) {
      ...
      this.props.dispatch(wdPopupActions.myaccountWdTransPosts(params,function(){
      ...
      this.props.dispatch(wdActions.getServerTime());
      this.props.dispatch(wdPopupActions.myaccountWdCancelTransPosts(params,function(){
        _this.props.dispatch(wdPopupActions.popupSetStatus({
          isShow: true,
          type: 'wd-revoke',
          title: item.bidTitle,
  ...
  render() {
    ...
    list = data.list.map(function(item,index){
          ...
          switch (item.transferStatus){
            case 1:
              transBtn = <td  onClick={() => {_this.transFn(item)}}>
                              ...
                              <tr>
                                  <td>
                                      <div className="number relative">
                                        <a href={url} className="font-arial font-size-base word-wrap" title={item.bidTitle}>{item.bidTitle}</a>
                                        {titleIcon}
                                      </div>
                                      {
                                        !item.selfTransfer
                                        ?
                                        <a href={item.agreementUrl} target="_blank">查看{item.agreementName}</a>
                                        :
                                        null
                                      }
                                  </td>
