...
const lclistDrain = React.createClass({
  getInitialState(){
    return{
      param:{
        pageNo:1,
  ...
  componentWillMount:function(){
    this.props.dispatch(lcActions.myaccountLcDrainPosts(this.state.param))
  },
  ...
  render:function(){
    // console.log(this.props.state.lcData.tabDrainList.data,1)Drain
    // let data = this.props.state.lcData.tabDrainList.data ? this.props.state.lcData.tabDrainList.data.data : {};
    // console.log(data,2)
    let outHtml = '', outList = '',innerList = '';
    let _this = this;
    if(this.props.state.lcData.tabDrainList.isFetching == 1){
      let data = this.props.state.lcData.tabDrainList.data.data;
      if(this.props.state.lcData.tabDrainList.data.errorCode == 0){
        if(!this.props.state.lcData.tabDrainList.data.data.list.length){
          outHtml = <div className="sydinlist-notips">
                        <span className="imgbox1">
                            <img src="https://static.huli.com/images/account.png" alt=""/>
                        </span>
                        <span>暂无已流标项目，<a href="https://www.huli.com/hl/#/invest/">先去认购</a>吧!</span>
                    </div>
        }else{
          outHtml = <div>
                      <div className="sydinlist-th">
                        ...
                        <span className="grid-wh-100 right-justified">认购时间</span>
                        ...
                        <span className="grid-wh-100 right-justified">认购金额</span>
                        <span className="grid-wh-20">&nbsp;</span>
    ...
    }else if(this.props.state.lcData.tabDrainList.isFetching == 0){
...
export const LclistDrain = connect(
