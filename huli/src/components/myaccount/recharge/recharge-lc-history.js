...
export const LCHistory = React.createClass({
    getInitialState(){
        return {
            startTime:'不限',
            endTime:'不限',
    ...
    render(){
        ...
        return (
            <div id="his-licai-list" style={{'padding':'0 30px'}}>
                <div className="curact-screening margin-md-vertical">
                    ...
                    <span className="curact-screening-item md-tp14"  style={{'zoom':1}}>
                        ...
                        <span className="time type-data">
                            <input type="text" data-target="start" value={this.state.startTime} className="time-start" id="beginTime2" readOnly />
                            <label>-</label>
                            <input type="text" data-target="last" value={this.state.endTime}  className="time-start" id="endTime2" readOnly />
                ...
                <div id="charge-detail-main">
                    {
                        !this.state.loading&&this.state.list.map( (item,index) => {
                            ...
                            return <div key={index} className="acgral-xs relative" data-opid={obj.chargeOpId} >
    ...
    componentDidMount(){
        $('#beginTime2,#endTime2').datepicker({
            format: 'yyyy-mm-dd',
            weekStart: 1,
            autoclose: true,
            todayBtn: false,
        ...
        $('#beginTime2').on('changeDate',(ev) => {
            if ($('#beginTime2').val() > $('#endTime2').val() && $('#endTime2').val() != '') {
                alert("生效时间 不能晚于 失效时间 ！");
                $("#beginTime2").val($("#endTime2").val());
            }else {
                this.setState({startTime: $("#beginTime2").val()},()=>{
                    this.getLCData()
                })
            }

        })
        $('#endTime2').on('changeDate',(ev) => {
            if ($('#beginTime2').val()!="不限"&&$('#beginTime2').val() > $('#endTime2').val() && $('#endTime2').val() != '') {
                alert("失效时间 不能早于 生效时间 ！");
                $("#endTime2").val($("#beginTime2").val());
            }else {
                this.setState({endTime: $("#endTime2").val()} ,() =>{
                    this.getLCData()
                })
            }
        })
    ...
    getLCData(){
        ...
        $.get({
            url:'/myaccount/capital/finDeposit',
            data:{
