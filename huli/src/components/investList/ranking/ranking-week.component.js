...
const Week = React.createClass({
  ...
  render:function(){
    ...
    if(this.props.rankingList.isFetching == 1){
      if(this.props.rankingList.data.week.length == 0){
        content = <div className="empty-list-detail current">
          <div className="empty-img"></div>
          <p>即将公布 敬请期待</p>
        </div>;
      }else{
        list1 = this.props.rankingList.data.week.map(function(item,index){
          if(index<5){
            let level = '';
            if(index == 0){
        ...
        list2 = this.props.rankingList.data.week.map(function(item,index){
          if(index>=5 && index<10){
            return (
              <div className="list-item cf" key={index}>
...
export const RankingWeek = connect(
  mapStateToProps
)(Week);